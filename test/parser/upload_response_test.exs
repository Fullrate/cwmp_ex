defmodule CWMP.Protocol.Parser.UploadResponseTest do
  ...
  @sample_result {:ok,
                  %{
                    cwmp_version: "1-0",
                    entries: [
                      %CWMP.Protocol.Messages.UploadResponse{
                        status: 1,
                        complete_time: generate_datetime({{19, 1, 2015}, {23, 9, 24}}),
                        start_time: generate_datetime({{19, 1, 2015}, {23, 8, 24}})
                      }
                    ],
                    header: %CWMP.Protocol.Messages.Header{
                      hold_requests: false,
                      id: "API_aa0642e34b23820801e7642ad7cb536c",
                      session_timeout: 30,
  ...
  test "raise, invalid status" do
    assert(
      catch_error(CWMP.Protocol.Parser.parse!(@sample2)) == %RuntimeError{
        message: "Integer does not validate"
      }
    )
  end
  ...
  @sample_result4 {:ok,
                   %{
                     ...
                     entries: [
                       %CWMP.Protocol.Messages.UploadResponse{
                         status: 1,
                         start_time: generate_datetime({{1, 1, 1}, {0, 0, 0}}),
                         complete_time: generate_datetime({{19, 1, 2015}, {23, 9, 24}})
                       }
                     ],
                     header: %CWMP.Protocol.Messages.Header{
  ...
  test "parses UploadResponse request, invalid StartTime" do
    assert(CWMP.Protocol.Parser.parse(@sample4) == @sample_result4)
  end
end
