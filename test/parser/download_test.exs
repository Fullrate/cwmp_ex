...
<SOAP-ENV:Envelope
  ...
  test "raise, invalid status" do
    assert(catch_error(CWMP.Protocol.Parser.parse(@sample2))==%RuntimeError{message: "Invalid status value"})
  end
...
<SOAP-ENV:Envelope
  ...
  test "parses DownloadResponse request, invalid StartTime" do
    assert(catch_error(CWMP.Protocol.Parser.parse(@sample4))==%RuntimeError{message: "StartTime 'foo' has unacceptable format"})
  end
