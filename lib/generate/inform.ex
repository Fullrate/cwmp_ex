defimpl CWMP.Protocol.Generate, for: CWMP.Protocol.Messages.Inform do
  import XmlBuilder
  ...
  defp events( informevents ) do
    eventlist=for e <- informevents, do: element(
      :EventStruct, [
        element(:EventCode, e.code),
        element(:CommandKey, e.command_key) ])
    element(:Event, %{"SOAP-ENC:arrayType": "cwmp:EventStruct[#{length(eventlist)}]"}, [eventlist])
  end

  defp parameters( parameterlist ) do
    plist=for p <- parameterlist, do: element(:ParameterValueStruct, [
      element(:Name, p.name),
      element(:Value, %{"xsi:type": p.type}, p.value) ] )
